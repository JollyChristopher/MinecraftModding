...
public class GuiBuffs extends Gui
{
	private Minecraft mc;

	public GuiBuffs(Minecraft mc)
	{
		super();
	...
	public void onRenderExperienceBar(RenderGameOverlayEvent event)
	{
		...
		int xPos = res.getScaledWidth() - BUFF_ICON_SIZE - 2;
		int yPos = 2;
		...
		if (!collection.isEmpty())
		{
			...
			for (Iterator iterator = this.mc.thePlayer.getActivePotionEffects().iterator(); iterator.hasNext(); 
					xPos -= BUFF_ICON_SPACING)
			{
				...
				if (potion.hasStatusIcon())
				{
					...
					this.drawTexturedModalRect(xPos, yPos, BUFF_ICON_BASE_U_OFFSET + iconIndex % BUFF_ICONS_PER_ROW
							* BUFF_ICON_SIZE,
