...
public class SpellMageShieldTest {
	private static SpellMageShield spell;

	@BeforeClass
	public static void init() {
		spell = (SpellMageShield) EnchantingMod.getSpell(SpellMageShield.enchID);
	}

	@Test
	public void testGetColor() {
		assertEquals("Color: absorption", Potion.absorption.getLiquidColor(), spell.getColor());
	}

	@Test
	public void testMaxLevel() {
		assertEquals("Max level", 5, spell.getMaxLevel());
	}

	@Test
	public void testGetMinEnchantability() {
		assertEquals("Level 1", 1, spell.getMinEnchantability(1));
		assertEquals("Max Level", 29,
				spell.getMinEnchantability(spell.getMaxLevel()));
	}

	@Test
	public void testGetMaxEnchantability() {
		assertEquals("Level 1", 10, spell.getMaxEnchantability(1));
		assertEquals("Max Level", 50,
				spell.getMaxEnchantability(spell.getMaxLevel()));
	}

	@Test
	public void testGetCastTime() {
		assertEquals("Cast time", 8, spell.getCastTime());
	}

	@Test
	public void testGetRawDescription() {
		assertEquals("Length", 1, spell.getRawDescription().length);
		assertEquals("Description[0]", "enchantment.mageshield.desc.1",
	...
	public void testGetRadius() {
		...
		assertEquals("Max Level", 20.0, spell.getRadius(spell.getMaxLevel()), 0.1);
	}
	...
	public void testGetRenderName(){
		assertEquals("Render name", "mageshield", spell.getRenderName());
	}
