...
public class SpellFireBallTest {
	private static SpellFireBall spell;

	@BeforeClass
	public static void init() {
		spell = (SpellFireBall) EnchantingMod.getSpell(SpellFireBall.enchID);
	}

	@Test
	public void testGetColor() {
		assertEquals("Color: red", 0xFFA500, spell.getColor());
	}
	...
	public void testGetRawDescription() {
		...
		assertEquals("Description[0]", "enchantment.fireball.desc.1",
	...
	public void testOnImpact() {
		...
		verify(thrower, times(0)).attackEntityFrom((DamageSource) any(),
				anyFloat());
		verify(thrower, times(0)).setFire(SpellFireBall.DURATION);
		verify(entity, times(0)).attackEntityFrom((DamageSource) any(),
				anyFloat());
		verify(entity, times(0)).setFire(SpellFireBall.DURATION);
		...
		verify(thrower, times(1)).setFire(SpellFireBall.DURATION);
		verify(entity, times(0)).attackEntityFrom((DamageSource) any(),
				anyFloat());
		verify(entity, times(0)).setFire(SpellFireBall.DURATION);
		...
		verify(thrower, times(2)).setFire(SpellFireBall.DURATION);
		verify(entity, times(0)).attackEntityFrom((DamageSource) any(),
				anyFloat());
		verify(entity, times(0)).setFire(SpellFireBall.DURATION);
		...
		verify(thrower, times(3)).setFire(SpellFireBall.DURATION);
		verify(entity, times(0)).attackEntityFrom((DamageSource) any(),
				anyFloat());
		verify(entity, times(0)).setFire(SpellFireBall.DURATION);
	}
