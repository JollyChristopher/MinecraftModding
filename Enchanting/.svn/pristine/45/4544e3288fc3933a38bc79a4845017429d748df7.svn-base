...
public class SpellDecayTest {
	private static SpellDecay spell;

	@BeforeClass
	public static void init() {
		spell = (SpellDecay) EnchantingMod.getSpell(SpellDecay.enchID);
	}

	@Test
	public void testGetColor() {
		assertEquals("Color: wither", Potion.wither.getLiquidColor(), spell.getColor());
	}
	...
	public void testGetMinEnchantability() {
		assertEquals("Level 1", 9, spell.getMinEnchantability(1));
		assertEquals("Max Level", 27,
				spell.getMinEnchantability(spell.getMaxLevel()));
	}

	@Test
	public void testGetMaxEnchantability() {
		assertEquals("Level 1", 18, spell.getMaxEnchantability(1));
		assertEquals("Max Level", 54,
	...
	public void testGetRawDescription() {
		assertEquals("Length", 2, spell.getRawDescription().length);
		assertEquals("Description[0]", "enchantment.decay.desc.1",
				spell.getRawDescription()[0]);
		assertEquals("Description[1]", "enchantment.decay.desc.2",
				spell.getRawDescription()[1]);
	}
